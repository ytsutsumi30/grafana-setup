...
w        });
w    } catch (error) {
        lwogger.error('Database connection error:', error);
        rwes.status(500).json({ error: 'Database connection failed' });
    }w
});w
w
w// === 製品関連API ===
w
// バリデーションスキーマw
const prwoductSchema = Joi.object({
    prodwuct_code: Joi.string().max(50).required(),
    product_name: Joiw.string().max(255).required(),
    descrwiption: Joi.string().allow('', null),
...
app.get('/reports/daily-inspection-stats', async (req, res) => {
    try {
        const result = await pool.query(`
            SELECT
                COUNT(*) FILTER (WHERE si.status = 'completed' AND DATE(si.updated_at) = CURRENT_DATE) as completed_today,
                COUNT(*) FILTER (WHERE si.status = 'pending') as in_progress,
                COUNT(*) FILTER (WHERE si.status = 'rejected' AND DATE(si.updated_at) = CURRENT_DATE) as failed_today,
                COUNT(*) FILTER (WHERE si.status = 'completed' AND DATE(si.updated_at) = CURRENT_DATE) * 100.0 /
                    NULLIF(COUNT(*) FILTER (WHERE (si.status = 'completed' OR si.status = 'rejected') AND DATE(si.updated_at) = CURRENT_DATE), 0) as pass_rate_today
            FROM shipping_instructions si
        `);
